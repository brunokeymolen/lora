...
void LoRaConfig(uint8_t spreadingFactor, uint8_t bandwidth, uint8_t codingRate, uint16_t preambleLength, uint8_t payloadLen, bool crcOn, bool invertIrq) 
{
	...
	/* Low Data Rate Optimize: required when symbol time > 16 ms.
	 * At BW=125 kHz: symbol_ms = 2^SF / 125000 * 1000 → enable for SF>=11. */
	uint8_t ldro = (spreadingFactor >= 11) ? 1 : 0;
...
void SetRxEnable(void)
{
	...
	/* Drive each GPIO independently – TXEN-only configs (e.g. Heltec V4)
	 * are valid: DIO2 controls the RF switch, TXEN enables the external PA. */
	if (SX126x_TXEN != -1) gpio_set_level(SX126x_TXEN, LOW);
	if (SX126x_RXEN != -1) gpio_set_level(SX126x_RXEN, HIGH);
}
...
void SetTxEnable(void)
{
	...
	/* Drive each GPIO independently – TXEN-only configs (e.g. Heltec V4)
	 * are valid: DIO2 controls the RF switch, TXEN enables the external PA. */
	if (SX126x_RXEN != -1) gpio_set_level(SX126x_RXEN, LOW);
	if (SX126x_TXEN != -1) gpio_set_level(SX126x_TXEN, HIGH);
}
